...
orig, output = display_samples("generated/models/wcgan-gp/final_gen.keras", image_path=image_path, save_path=save_path, data_path="data/frames/")
# orig = cv2.resize(orig, None, fx=.5, fy=.5)
if output.shape[0] == 480 and output.shape[1] == 270:
        output = np.transpose(output, (1, 0, 2))
# process output to be able to be fed into the discriminator
output = (output - 127.5) / 127.5
output = np.expand_dims(output, axis=0)

discriminator = load_model("generated/models/wcgan-gp/final_disc.keras")
